  ...
  if (req.url.startsWith('/api/dashboard-stats')) {
    ...
    try {
      const [[sRows], [rRows], [tRows]] = await Promise.all([
        ...
        p.query(`SELECT COUNT(*) AS total, SUM(CASE WHEN IFNULL(c.claim_status,'')='sent' THEN 1 ELSE 0 END) AS sent, SUM(CASE WHEN IFNULL(c.claim_status,'')!='sent' THEN 1 ELSE 0 END) AS pending, SUM(CASE WHEN c.clinic_hd_confirm_status='ready' AND IFNULL(c.claim_status,'')!='sent' THEN 1 ELSE 0 END) AS ready, SUM(CASE WHEN IFNULL(c.clinic_hd_confirm_status,'')!='ready' AND IFNULL(c.claim_status,'')!='sent' THEN 1 ELSE 0 END) AS unreviewed, SUM(CASE WHEN IFNULL(c.claim_status,'')='sent' AND IFNULL(c.clinic_hd_claim_sent_by,'')!='' THEN 1 ELSE 0 END) AS auto_sent, IFNULL(SUM(CASE WHEN IFNULL(c.claim_status,'')='sent' THEN IFNULL(c.clinic_hd_claim_amount,0) ELSE 0 END),0) AS total_amount FROM clinic_hd_claim c INNER JOIN ovst v ON v.vn=c.vn WHERE v.vstdate BETWEEN ${sqlEsc(from)} AND ${sqlEsc(to)}`),
        ...
        p.query(`SELECT COUNT(*) AS total, SUM(CASE WHEN IFNULL(c.claim_status,'')='sent' THEN 1 ELSE 0 END) AS sent, SUM(CASE WHEN c.clinic_hd_confirm_status='ready' AND IFNULL(c.claim_status,'')!='sent' THEN 1 ELSE 0 END) AS ready, SUM(CASE WHEN IFNULL(c.clinic_hd_confirm_status,'')!='ready' AND IFNULL(c.claim_status,'')!='sent' THEN 1 ELSE 0 END) AS unreviewed, IFNULL(SUM(CASE WHEN IFNULL(c.claim_status,'')='sent' THEN IFNULL(c.clinic_hd_claim_amount,0) ELSE 0 END),0) AS total_amount_today FROM clinic_hd_claim c INNER JOIN ovst v ON v.vn=c.vn WHERE v.vstdate=CURDATE()`),
      ]);
